...
- `RadioHealth` (`*DiagnosticsRadioHealth`): what the radio reports about its own transmit path. `Supported` is false when the driver has no SX1262 to interrogate, and only `TxPowerDBm` is populated then; every other field stays nil. `OCPOK` is the check for PA configuration writes not reaching the chip, and a present false there is a fault report, which is exactly why an absent field must not decode to false.
...
if diag.RadioHealth != nil && diag.RadioHealth.Supported {
    if diag.RadioHealth.OCPOK != nil && !*diag.RadioHealth.OCPOK {
        fmt.Println("PA configuration writes are not reaching the chip")
    }
}
